...
if ($search) {
    $sql .= " AND (title LIKE :search OR tech_stack LIKE :search)";
...
<head>
    <meta charset="UTF-8">
    <title>Kelola Project - Main Kode</title>
...
<body class="bg-gray-950 text-white min-h-screen flex flex-col">
    ...
    <nav class="bg-gray-900 border-b border-white/10 p-4 flex justify-between items-center md:hidden">
        <h1 class="text-xl font-bold text-white">Kelola Project</h1>
    ...
    <div class="flex flex-1">
        ...
        <main class="flex-1 p-6 space-y-6">
            ...
            <div class="flex justify-between items-center">
                ...
                <a href="project_tambah.php" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded text-sm">
            ...
            <form class="flex flex-wrap gap-4 items-center" method="GET">
                <input type="text" name="search" placeholder="Cari judul atau tech stack..."
                    ...
                    <tbody class="divide-y divide-white/10">
                        <?php foreach ($projects as $i => $project): ?>
                            <tr class="hover:bg-white/5 transition">
                                ...
                                <td class="font-medium"><?= htmlspecialchars($project['title']) ?></td>
                                <td>
                                    <?php if (!empty($project['image'])): ?>
                                        <img src="../../backend/uploads/projects/<?= htmlspecialchars($project['image']) ?>" alt="" class="h-12 w-12 object-cover rounded">
                                    <?php else: ?>
                                        <span class="text-gray-400 italic">No Image</span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-gray-300"><?= htmlspecialchars($project['tech_stack']) ?></td>
                                <td>
                                    <?php if (!empty($project['link'])): ?>
                                        <a href="<?= htmlspecialchars($project['link']) ?>" target="_blank" class="text-blue-400 underline">Lihat</a>
                                    <?php else: ?>
                                ...
                                <td>
                                    <a href="project_edit.php?id=<?= $project['id'] ?>" class="text-yellow-400 hover:underline text-xs">
                                        <i class="bi bi-pencil-square"></i> Edit
                                    </a> |
                                    <a href="../../backend/admin/projects_hapus.php?id=<?= $project['id'] ?>"
                        ...
                        <?php if (count($projects) === 0): ?>
                            <tr>
                                <td colspan="7" class="text-center py-4 text-gray-400 italic">Tidak ada project ditemukan.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
